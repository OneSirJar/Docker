## Note: This code was entirely borrowed (with permission) from Ties Noordhuis with edits made as required.
...
VALUES (
        1,
        'Explain the underlying concepts, principles, and steps needed to approach and solve this problem, without giving direct answers or writing the code. Focus on helping me understand why specific approaches, patterns, or structures are effective, and outline general strategies or pitfalls to consider. Offer conceptual guidance, best practices, and examples in plain language so I can apply these ideas independently to write the solution myself.',
        'Explain concepts to solve exercises and to understand the underlying principles.'
    ),
    (
        1,
        'Make sure to ask me additional questions or request further context if it would improve the response quality or make it more specific to my needs and tailored to my situation.',
        'Ask extra questions and context.'
    ),
    (
        1,
        'When explaining concepts or discussing languages and frameworks, please reference the most recent, stable versions only. Use modern syntax and standards, such as ES6, PHP8, Python3.12 and other contemporary frameworks and libraries, avoiding outdated versions like Bootstrap 4 or older syntaxes such as mysqli. Ensure explanations reflect best practices and syntax aligned with current documentation.',
        'Use modern syntax and standards.'
    ),
    (
        1,
        'I need to find the most common value in an array in javascript.',
...
INSERT INTO
    prompt_fragment_tag (prompt_fragment_id, tag_id)
VALUES (1, 1),
    (2, 2),
    (3, 1),
    (4, 1),
...
INSERT INTO
    composite_prompt (author_id, title, description)
VALUES (
        1,
        'JavaScript: Find the most common value in an array',
        'Combine the provided prompt fragments to create a composite prompt for a JavaScript exercise.'
    );

INSERT INTO
    composite_prompt_prompt_fragment (
        composite_prompt_id,
        prompt_fragment_id,
        order_index
    )
...
VALUES (
        1,
        "To find the most common value in an array in JavaScript, it's essential to understand some fundamental concepts and principles related to data structures, algorithms, and the JavaScript language itself. Here’s a structured approach to help you tackle this problem independently:
...
VALUES (
        1,
        'array.js',
        'context_files/array.js',
        'Example array for the JavaScript exercise.'
    );