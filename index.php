    ...
    <?php if (!isset($_POST['_submit'])) {?>
        <form method="post" action="">
            <select class="tags" id="tags" name="tags">
                ...
                <option value="<?= $tag['id']?>"><button name="<?= $id = $tag['id'];?>" value="<?= $tag['id']?>"><?= $tag['name']?></button></option>
                <?php }?>
            </select>
            <input type="text" id="searchInput" class="search-input" placeholder="Search here...">
            <button onclick="performSearch()" class="search-button" type="submit">Search</button>
        </form>
        <?php }?>
    </div>
</div>
